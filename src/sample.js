...
Platform.OS === "ios"
      ...
      if (nextImageTensor && cameraActiveRef.current) {
        try {
          ...
          if(predictions&&predictions.length>0){
            ...
            const processlandmark= preprocessLandmarks(predictions)
            const predictvalue=await model.predict(processlandmark)
            const maxProbabilityIndex = predictvalue.indexOf(Math.max(...predictvalue))
            const predictedLetter = class_names[maxProbabilityIndex];
            setCurr(predictedLetter);
            setPredictions(prevPredictions => [...prevPredictions, predictedLetter])
          }
        }
        catch (error) {
  ...
  // Flatten the image to convert it into a 1D tensor
  const flattenedImage = normalizedImage.flatten();
  // Ensure the flattened image has 42 elements
  const paddedImage = tf.pad(flattenedImage, [[0, 42 - flattenedImage.size]]);
  // Reshape the flattened image to have shape [1, 42]
  const reshapedImage = paddedImage.reshape([1, 42]);
  return reshapedImage;
};


  const detectHandLandmarks = async (imageTensor) => {
    const predictions = await handposeModel.estimateHands(imageTensor);
    console.log(predictions)
    return predictions;
  };

  const preprocessLandmarks = (landmarks) => {
    const NUM_LANDMARKS = 21; // Number of landmarks
    const NUM_COORDS = 2; // Number of coordinates (x, y, z) per landmark
  
    // Flatten the landmarks array and normalize coordinates
    const flattenedLandmarks = landmarks[0].landmarks.flat();
  
    // Calculate the maximum absolute value among all landmark coordinates
    const maxAbsValue = Math.max(...flattenedLandmarks.map(Math.abs));
  
    // Normalize the landmark coordinates
    const normalizedLandmarks = flattenedLandmarks.map((coord) => coord / maxAbsValue);
  
    // Trim to the first 42 elements to match the expected shape
    const trimmedLandmarks = normalizedLandmarks.slice(0, 42);
  
    // Convert the normalized and trimmed landmarks to a TensorFlow tensor
    const preprocessedTensor = tf.tensor2d([trimmedLandmarks], [1, NUM_LANDMARKS * NUM_COORDS]);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.text}>
        <Text>Predicting letters: {curr}</Text>
      </View>
      <View style={styles.text}>
        <Text> Output: {predictions.join('')}</Text>
      </View>
    </SafeAreaView>
  );
}
